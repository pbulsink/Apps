...
query <- dbSendQuery(con, paste("SELECT ", paste(colvector, collapse = ", "), " FROM playerseason", sep = ""))
data <- fetch(query, -1)
...
shinyServer(function(input, output, session) {
  ...
  simdata <- reactive({
    ...
    distlist <- list(
      ((data$G60 - pdata$G60)/(max(data$G60) - min(data$G60)))^2,
      ((data$A60 - pdata$A60)/(max(data$A60) - min(data$A60)))^2,
      ((data$P60 - pdata$P60)/(max(data$P60) - min(data$P60)))^2,
      ((data$CF60 - pdata$CF60)/(max(data$CF60) - min(data$CF60)))^2,
      ((data$CA60 - pdata$CA60)/(max(data$CA60) - min(data$CA60)))^2,
      ((data$CF. - pdata$CF.)/(max(data$CF.) - min(data$CF.)))^2,
      ((data$Rel.CF60 - pdata$Rel.CF60)/(max(data$Rel.CF60) - min(data$Rel.CF60)))^2,
      ((data$Rel.CA60 - pdata$Rel.CA60)/(max(data$Rel.CA60) - min(data$Rel.CA60)))^2,
      ((data$Rel.CF. - pdata$Rel.CF.)/(max(data$Rel.CF.) - min(data$Rel.CF.)))^2,
      ((data$xGF60 - pdata$xGF60)/(max(data$xGF60) - min(data$xGF60)))^2,
      ((data$xGA60 - pdata$xGA60)/(max(data$xGA60) - min(data$xGA60)))^2,
      ((data$xGF. - pdata$xGF.)/(max(data$xGF.) - min(data$xGF.)))^2,
      ((data$Rel.xGF60 - pdata$Rel.xGF60)/(max(data$Rel.xGF60) - min(data$Rel.xGF60)))^2,
      ((data$Rel.xGA60 - pdata$Rel.xGA60)/(max(data$Rel.xGA60) - min(data$Rel.xGA60)))^2,
      ((data$Rel.xGF. - pdata$Rel.xGF.)/(max(data$Rel.xGF.) - min(data$Rel.xGF.)))^2,
      ((data$GF60 - pdata$GF60)/(max(data$GF60) - min(data$GF60)))^2,
      ((data$GA60 - pdata$GA60)/(max(data$GA60) - min(data$GA60)))^2,
      ((data$GF. - pdata$GF.)/(max(data$GF.) - min(data$GF.)))^2,
      ((data$Rel.GF60 - pdata$Rel.GF60)/(max(data$Rel.GF60) - min(data$Rel.GF60)))^2,
      ((data$Rel.GA60 - pdata$Rel.GA60)/(max(data$Rel.GA60) - min(data$Rel.GA60)))^2,
      ((data$Rel.GF. - pdata$Rel.GF.)/(max(data$Rel.GF.) - min(data$Rel.GF.)))^2,
      ((data$iCF60 - pdata$iCF60)/(max(data$iCF60) - min(data$iCF60)))^2,
      ((data$ixG60 - pdata$ixG60)/(max(data$ixG60) - min(data$ixG60)))^2,
      ((data$ixFSh. - pdata$ixFSh.)/(max(data$ixFSh.) - min(data$ixFSh.)))^2,
      ((data$TOI. - pdata$TOI.)/(max(data$TOI.) - min(data$TOI.)))^2,
      ((data$TOI.QoT - pdata$TOI.QoT)/(max(data$TOI.QoT) - min(data$TOI.QoT)))^2,
      ((data$CF.QoT - pdata$CF.QoT)/(max(data$CF.QoT) - min(data$CF.QoT)))^2,
      ((data$xGF.QoT - pdata$xGF.QoT)/(max(data$xGF.QoT) - min(data$xGF.QoT)))^2,
      ((data$TOI.QoC - pdata$TOI.QoC)/(max(data$TOI.QoC) - min(data$TOI.QoC)))^2,
      ((data$CF.QoC - pdata$CF.QoC)/(max(data$CF.QoC) - min(data$CF.QoC)))^2,
      ((data$xGF.QoC - pdata$xGF.QoC)/(max(data$xGF.QoC) - min(data$xGF.QoC)))^2,
      ((data$ZSR - pdata$ZSR)/(max(data$ZSR) - min(data$ZSR)))^2,
      ((data$iPEND60 - pdata$iPEND60)/(max(data$iPEND60) - min(data$iPEND60)))^2,
      ((data$iPENT60 - pdata$iPENT60)/(max(data$iPENT60) - min(data$iPENT60)))^2,
      ((data$iHF60 - pdata$iHF60)/(max(data$iHF60) - min(data$iHF60)))^2,
      ((data$iTKA60 - pdata$iTKA60)/(max(data$iTKA60) - min(data$iTKA60)))^2,
      ((data$iGVA60 - pdata$iGVA60)/(max(data$iGVA60) - min(data$iGVA60)))^2,
      ((data$iBLK60 - pdata$iBLK60)/(max(data$iBLK60) - min(data$iBLK60)))^2,
      ((data$iFO. - pdata$iFO.)/(max(data$iFO.) - min(data$iFO.)))^2
    )
